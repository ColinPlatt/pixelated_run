...
const ONE_QUADRILLION = new BigNumber(1_000_000_000_000_000);
const ONE_TRILLION = new BigNumber(1_000_000_000_000);
const ONE_BILLION = new BigNumber(1_000_000_000);
const ONE_MILLION = new BigNumber(1_000_000);
// const ONE_HUNDRED_THOUSAND = new BigNumber(100_000)
const TEN_THOUSAND = new BigNumber(10_000);
const ONE_THOUSAND = new BigNumber(1_000);
// const ONE_HUNDRED = new BigNumber(100)
// const TEN = new BigNumber(10)
const ONE = new BigNumber(1);
const ONE_MILLIONTH = new BigNumber(0.000001);

function formatCryptoValUnder10K(amount: BigNumber) {
  if (amount.isLessThan(ONE_MILLIONTH)) return `<${ONE_MILLIONTH.toString()}`; // otherwise we"ll get output like "1e-18"
  if (amount.isLessThan(ONE))
    return amount.precision(6).decimalPlaces(6).toString();

  const formattedVal = amount.isInteger()
    ? amount.decimalPlaces(0)
    : amount.precision(6).decimalPlaces(2);

  return formattedVal.toNumber().toLocaleString();
}

function formatCryptoValFrom10Kto1Quadrillion(amount: BigNumber) {
  return amount.isGreaterThan(ONE_TRILLION)
    ? `${amount
        .dividedBy(ONE_TRILLION)
        .decimalPlaces(2)
        .toNumber()
        .toLocaleString()}T`
    ...
    ? `${amount
        .dividedBy(ONE_MILLION)
        .decimalPlaces(2)
        .toNumber()
        .toLocaleString()}M`
    : `${amount
        .dividedBy(ONE_THOUSAND)
        .decimalPlaces(2)
        .toNumber()
        .toLocaleString()}k`;
}
...
export function formatCryptoVal(
  ...
  return parsedamount.isGreaterThan(ONE_QUADRILLION)
    ...
    : parsedamount.isGreaterThanOrEqualTo(TEN_THOUSAND)
    ? formatCryptoValFrom10Kto1Quadrillion(parsedamount)
    : formatCryptoValUnder10K(parsedamount);
}
